...
df_gc_stats <- data.frame(fread('all_genomes.gc_table.txt'), na.string='')

colnames(df_gc_stats) <- c("id", "gene_gc", "flanking_1kb_gc", "flanking_5kb_gc")

df_gc_stats <- subset(df_gc_stats, select =  c("id", "gene_gc", "flanking_1kb_gc", "flanking_5kb_gc"))
...
output.file <- file('gene_info.genes_tes_gc.lengths_effectors.guy11_histone_expression.go_pfam_eccdnas.no_te_classes.txt', "wb") ## to get unix line endings
...
df_gene_info_cross_host <- subset(df_gene_info, select = -c(eccdna_cov, eccdnasr_cov, 
